...
this.REACT_SPEECH_INTERVAL = 3000;      // 2–4 seconds feels natural
this.AMBIENT_SPEECH_INTERVAL = 30000;   // idle chatter // 30s minimum between bubbles
  ...
  speak(text, duration = 4000, type = 'reactive') {
    ...
    if (type === 'ambient') {
        if (now - this.lastAmbientSpeech < this.AMBIENT_SPEECH_INTERVAL) {
            return;
        }
        this.lastAmbientSpeech = now;
    } else {
        if (now - this.lastReactiveSpeech < this.REACT_SPEECH_INTERVAL) {
            return;
        }
